...
**INDEXATOR** is a metadata management tool that addresses the problems of organising, documenting, storing and sharing data in a research unit or infrastructure, and fits perfectly into a structural data management plan.
...
PGD-MMDT uses 3 Docker images for 3 distinct services:
...
-  ***web/json/config_terms.json*** : define the terminology (see below)
-  ***web/json/pgd-mmdt-schema.json*** : define the JSON schema used to validate metadata files (see below)
...
- ***passwd*** : Define the admin password allowing you to copy the new configuration file on the server via the web interface (i.e config)
...
* The application can be sequentially started :
   ...
   * Initialization of the MongoDB database
     ...
     sh ./run initdb
     ```
   * Scanning the data directory for metadata files (PGD_XXXX.json)
     ```sh
     sh ./run scan
     ```
...
Terminology is the set of terms used to define the metadata of a project. A single file (***web/json/config_terms.json***) contains all the terminology. It is therefore necessary to configure it, according to the following scheme:
...
* The terminology file (***web/json/config_terms.json***) is structured as follows:
...
* Click on the '***Replace the current JSON files***' button to replace current JSON files under ***./web/json***. A password will be asked to you to perform this replacement. This also assumes that you have write rights to these files. In case you have no write rights to these files or an _Internal Server Error_ occurs, you must put these files manually under ***./web/json*** by yourself.
...
* **Notes**: 
   ...
   3. **autocomplete** : In this release, we have implemented the possibility to put vocabulary:
      ...
      * from VO Thesaurus-INRAE (https://thesaurus.inrae.fr/thesaurus-inrae/fr/). See the VOvocab.js script as an example of usage,
